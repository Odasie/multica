...
func BuildPrompt(task Task, provider string) string {
	...
	fmt.Fprintf(&b, "If you need comment history, `multica issue comment list %s --output json` returns all comments for the issue (server caps at 2000). On long-running issues prefer `--recent 20 --output json` to grab the 20 most recently active threads instead of the full flat dump (the stderr `Next thread cursor: ...` line feeds `--before` / `--before-id` to scroll older threads). `--since <RFC3339>` is still available for incremental polling and may combine with `--recent`.\n", task.IssueID)
	return b.String()
}
