...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		fmt.Fprintf(&b, "2. Run `multica issue comment list %s --output json` to read the full comment history (returns all comments, capped server-side at 2000) — this is mandatory, not optional. Earlier comments often carry context the issue body lacks (e.g. which repo to work in, the prior agent's findings, the reason the issue was reassigned to you). Skipping this step is the most common cause of agents acting on stale or incomplete instructions. On a long-running issue you may switch to `--recent 20 --output json` (with the `--before` / `--before-id` cursor from the stderr `Next thread cursor:` line to scroll older threads) — that returns the same comment shape but grouped into the most recently active threads, which keeps your prompt focused on conversations that are still moving.\n", ctx.IssueID)
